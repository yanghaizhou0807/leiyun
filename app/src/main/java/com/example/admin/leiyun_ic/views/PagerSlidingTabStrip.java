 ...
 */
package com.example.admin.leiyun_ic.views;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int tabPaddingTopBottom = 24;
    private int dividerWidth = 1;
    private int tabTextSize = 12;
    private int tabTextColor = 0xFF666666;
    private boolean isHasLineOffset = true;
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabPaddingLeftRight = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsTabPaddingLeftRight,
                tabPaddingLeftRight);
        tabPaddingTopBottom = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsTabPaddingTopBottom,
                tabPaddingTopBottom);
        //        tabBackgroundResId = a.getResourceId(R.styleable.PagerSlidingTabStrip_pstsTabBackground,
        // tabBackgroundResId);
        shouldExpand = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsShouldExpand, shouldExpand);
        isHasLineOffset = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsLineOffset, isHasLineOffset);
    ...
    public void notifyDataSetChanged() {
        ...
        int textLength = (int) paint.measureText(maxLengthText);
        if (shouldExpand) {
            if(!isHasLineOffset){
                offset=0;
            }else{
                offset = (BaseApplication.getInstance().mScreenWidth / tabCount - textLength) / 4;
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!shouldExpand) {
            if (currentTab instanceof TextView) {
                ...
                int textLength = (int) tab.getPaint().measureText(tab.getText().toString());
                offset = (tab.getWidth() - textLength) / 2;
            }
        }
        float lineLeft = currentTab.getLeft() + offset;
        float lineRight = currentTab.getRight() - offset;

        // if there is an offset, start interpolating left and right coordinates between current and next tab
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {

            View nextTab = tabsContainer.getChildAt(currentPosition + 1);
            final float nextTabLeft = nextTab.getLeft() + offset;
            final float nextTabRight = nextTab.getRight() - offset;
